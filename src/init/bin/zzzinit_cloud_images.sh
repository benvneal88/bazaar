...
# ansible is a better method to build the termplate than ssh on the pve host. see configure_vm_templates.yml -ben
...
TARGET_DIRECTORY="/media/images/"

# Download the disk image
mkdir "$TARGET_DIRECTORY"
rm -f "$DISK_IMAGE"
wget -q "$IMAGE_URL" -P "$TARGET_DIRECTORY"
...
sudo qm create 9022 --name "$IMAGE_TEMPLATE_NAME" --memory 2048 --cores 2 --net0 virtio,bridge=vmbr0
